...
def extract_textures(tex_path: Path, output_dir: Path | None = None) -> list[Path]:
    ...
    for i, texture in enumerate(info.textures):
        ...
        data_to_write = texture.data
        
        if texture.format == TextureFormat.RAW:
            ext = "bmp"
            try:
                # convert_raw_to_bmp expects the raw RLE data (or uncompressed data)
                # and creates a valid BMP file byte string.
                data_to_write = convert_raw_to_bmp(texture.data, texture.width, texture.height)
            except Exception as e:
                print(f"Failed to convert RAW to BMP for {base_name}: {e}")
        ...
        # Only extract the highest resolution texture (first one)
        break
